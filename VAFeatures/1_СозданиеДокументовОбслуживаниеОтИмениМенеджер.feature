...
*Создаю новый документ "Обслуживание клиента"
	...
	* Создаю или выбираю элемент справочника "Договоры контрагентов"
	...
	Если в таблице "Список" нет строки Тогда
		...
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
		И я жду закрытия окна 'Договор контрагента (создание) *' в течение 10 секунд
	И в таблице "Список" поле "Наименование" имеет значение "Абонентский договор № 2 от 01.07.2025 г." тогда
	И я нажимаю на кнопку с именем 'ФормаВыбрать'
	Тогда открылось окно 'Обслуживание клиента (создание) *'
	* Выбираю "Специалиста" из списка справочника "Пользователи"
	И я нажимаю кнопку выбора у поля с именем "Специалист"
...
*Создаю новый документ "Обслуживание клиента"
	...
	* Создаю или выбираю элемент справочника "Договоры контрагентов"
	...
	Если в таблице "Список" нет строки Тогда
		| 'Наименование' 							 |
		| 'Абонентский договор № 3 от 01.07.2025 г.' |
		И я нажимаю на кнопку с именем 'ФормаСоздать'
		Тогда открылось окно 'Договор контрагента (создание)'
		И из выпадающего списка с именем "ВидДоговора" я выбираю точное значение 'Абонент'
	...
	* Проверяю выполнение сценария тестировавния
	Тогда таблица "Список" содержит строки по шаблону:
		...
		| '9:00:00'                   | '15:00:00'                     | '02.07.2025' | '3 июля 2025 г.'        | 'Абонентский договор № 2 от 01.07.2025 г.' | 'ИП Степ А.Н.'  | 'Тел. менеджера клиента: +7 123 (456) 7890.' | '000000002' | 'Клиент просит подобрать ему сервер.'                 |
		| '10:00:00'                  | '16:00:00'                     | '03.07.2025' | '4 июля 2025 г.'       | 'Абонентский договор № 3 от 01.07.2025 г.' | 'ООО "Антон"'    | 'Тел. менеджера клиента: +7 123 (456) 7890.' | '000000003' | 'Решить проблему.'                                    |
